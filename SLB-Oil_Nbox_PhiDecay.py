...
TotalThickness = 50
StartPhi = Parameter(name='Maximum Volume Fraction', value=1.0).range(0.8,1.0)
...
ThicknessRange = [[MinThickness,40] for _ in range(NBox)]

RoughValue = np.full(NBox, AverageRough)
RoughRange = [[MinRough, MaxRough] for _ in range(NBox)]

PhiValue = np.full(NBox, 1.0)
PhiRange = [[0.0, 1.0] for _ in range(NBox)]

FracPhiValue = np.full(NBox, FracPhi)
FracPhiRange = [[MinFracPhi, MaxFracPhi] for _ in range(NBox)]
...
FracPhi = make_N_pars(NBox, 'Frational Volume Fraction', FracPhiValue, FracPhiRange)
...
if PhiMaxSolid:
    # loop out from the Solid interface
    Phi[0] = StartPhi
    cumFrac = StartPhi
    for i in range(1,NBox): 
        cumFrac = cumFrac * FracPhi[i]
        Phi[i] = cumFrac
        Phi[i].fixed = True
